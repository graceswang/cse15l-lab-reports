...
I use ``ssh cs15lfa23bo@ieng6.ucsd.edu`` command, then <enter>.
...
I use ``git clone git@github.com:graceswang/lab7.git`` command, then <enter>.
...
I use ``bash test.sh`` command, then <enter>. testMerge2 failed because test timed out after 500 milliseconds.
...
Use ``vim ListExamples.java`` command, then <enter>,to open the ListExamples.java at terminal.
...
Then use <down>*17, <right>, <delete>,change "1" into "2"
