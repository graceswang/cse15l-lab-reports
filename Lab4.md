...
Keys pressed: ``ssh cs15lfa23bo@ieng6.ucsd.edu`` command, ``<enter>``.
...
Keys pressed: ``git clone git@github.com:graceswang/lab7.git`` command, ``<enter>``.
...
Keys pressed: ``bash test.sh`` command, ``<enter>``. 
...
Keys pressed: ``vim ListExamples.java`` command,  ``<enter>``(to open the ListExamples.java at terminal).
...
Keys pressed:`` <down><down><down><down><down><down><down><down><down><down><down><down><down><down><down><down><down>``(intotoal 17),``<i>``, ``<right>``, ``<delete>``, change "1" into "2"
...
Keys pressed: ``git commit -a`` command, ``<enter>``
...
Keys pressed: ``git push`` command, ``<enter>``
